...
class Rover(object):
    ...
    def __init__(self):
        ...
        self.wheel_distance_width = 0.7 #[m]
        #self.spin_linear_x_thresh = 
        self.min_radius = 0.5  # [m]
        self.max_radius = 6.4  # [m]

        self.no_cmd_thresh = 0.05  # [rad]
        self.wheel_radius = rospy.get_param("/rover_dimensions/wheel_radius", 0.105)  # [m]
        drive_no_load_rpm = rospy.get_param("/drive_no_load_rpm", 90)
        ...
        self.left_pub = rospy.Publisher("/rover/left_targets",Float64MultiArray,queue_size=1)
        self.right_pub = rospy.Publisher("/rover/right_targets",Float64MultiArray,queue_size=1)
    ...
    def cmd_cb(self, twist_msg, intuitive=False):
        ...
        #旋回する場合、ローバーの中央部が進むことができる最大速度を計算する。
        ...
        velocity = min(max_vel, pow(twist_msg.linear.x**2+twist_msg.linear.y**2,0.5))
        rospy.logdebug_throttle(1, "velocity drive cmd: {} m/s".format(velocity))

        drive_cmd_msg = CommandDrive()
        #deviceの関係でlinear.x=0は存在しない
        # if twist_msg.linear.x == 0 and twist_msg.angular.z != 0:
        #     ang = twist_msg.angular.z
        #     drive_cmd_msg = self.calculate_drive_velocities_spin(ang)
        # else:
            #drive_cmd_msg = self.calculate_drive_velocities(velocity, desired_turning_radius)
        drive_cmd_msg = self.calculate_drive_velocities_karura_term2(twist_msg)
        
        left_msg = [drive_cmd_msg.left_front_vel]
        left_msg.append(drive_cmd_msg.left_middle_vel)
        left_msg.append(drive_cmd_msg.left_back_vel)

        right_msg = [drive_cmd_msg.right_front_vel]
        right_msg.append(drive_cmd_msg.right_middle_vel)
    ...
    def calculate_drive_velocities(self, speed, current_radius):
        ...
        希望する速度と現在の回転半径に基づいて、駆動モーターの目標速度を計算する。
        ...
        if speed == 0:
            ...
            #すべての車輪の中心の所望の速度を計算する。同じ側のコーナーホイール
            # v = r * ω再び
    ...
    def calculate_drive_velocities_karura_term2(self, twist):
        ...
        cmd_msg.left_back_vel = speed * left_wheel_direct
    ...
    # def calculate_drive_velocities_spin(self, angular_z):
    #     cmd_msg = CommandDrive()
    #     ang_vel_right = angular_z * self.d4 / self.wheel_radius
    #     ang_vel_left = -angular_z * self.d1 / self.wheel_radius

    #     cmd_msg.right_front_vel = ang_vel_right
    #     cmd_msg.right_middle_vel = ang_vel_right
    #     cmd_msg.right_back_vel = ang_vel_right

    #     cmd_msg.left_front_vel = ang_vel_left
    #     cmd_msg.left_middle_vel = ang_vel_left
    #     cmd_msg.left_back_vel = ang_vel_left
    #     return cmd_msg

    # def calculate_drive_velocities_karura_term1(self, current_radius, twist):
    #     """
    #     Calculate target velocities for the drive motors based on desired speed and current turning radius
    #     希望する速度と現在の回転半径に基づいて、駆動モーターの目標速度を計算する。
    ...
    #     ang_speed = twist.angular.z * self.wheel_distance_width

    #     #speed = max(-self.max_vel, min(self.max_vel, speed))
    #     cmd_msg = CommandDrive()

    #     angular_vel_right = (speed+ang_speed) / self.wheel_radius
    #     angular_vel_left = (speed-ang_speed) / self.wheel_radius
    #     cmd_msg.left_front_vel = angular_vel_left
    #     cmd_msg.left_middle_vel = angular_vel_left
    #     cmd_msg.left_back_vel = angular_vel_left
    #     cmd_msg.right_back_vel = angular_vel_right
    #     cmd_msg.right_middle_vel = angular_vel_right
    #     cmd_msg.right_front_vel = angular_vel_right
